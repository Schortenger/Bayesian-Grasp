...
def main():
    ...
    while grasp_time <= grasp_times:
        ...
        # Img_Name = "//home//schortenger//Desktop//IROS//tactile_prior//data//original/flashlight/x y theta:" + str(int(p1y))+str(int(p1x))+str(theta) +str(time.strftime('  %Y-%m-%d %H:%M:%S',time.localtime(time.time())))+ ".png"
        ...
        plt.imshow(rgb)
        #plt.show()
        # Img_Name = "//home//schortenger//Desktop//IROS//tactile_prior//data//background/flashlight/score:" + str(score) + str(
        #     time.strftime('  %Y-%m-%d %H:%M:%S', time.localtime(time.time()))) + ".png"
        ...
        # Img_Name = "//home//schortenger//Desktop//IROS//tactile_prior//data//processed/flashlight/score:" + str(score)+str(time.strftime('  %Y-%m-%d %H:%M:%S',time.localtime(time.time()))) + ".png"
        # cv2.imwrite(Img_Name, rgb)
...
#     timer=Timer()
#
#     # class of cnn network for predicting fruit grasping point and angle
#     # softgraspnet=SoftGraspNet(model1_input='rgb')
#     # class of realsense camera
#     # camera=Camera()
#     #class of image moment
#     # Grasp_Plan=GraspPlann()
#     # wrapper of UR5 to control movement
#     ur5=UR5(tcp_host_ip='192.168.1.2',tcp_port=30003)
#     # grasp home pose [0 -350 400 3.1415 0 0], change rotz to [-ang] for grasp, ang is output by network
#     # io 4 is close, 5 is open
#     # camera joint = [15,-111,111,-90,-90,161]
#     # camera joint position of capturing images
...
#     grasp_num = 0
#     while True:
#         # move ur5 to camera position to capture images
#         rgb_back, depth_back = Camera.get_data()
#         gray_back = Grasp_Plan.rgb2gray(rgb_back)
#         ur5.move_joints(cam_joint)
#         # recording time
#         timer.tic()
#         # get color image and depth image from realsense camera
#         rgb,depth=camera.get_data()
#         gray_img=Grasp_Plan.rgb2gray(rgb)
#         subtracted = gray_img - gray_back  # gray_img of grasping object
#         x_mean, y_mean, theta_image = grasp_plan.main_point_angle(subtracted)
#         #print("the output of main axis:", x_mean, y_mean, theta_image)
#         grasp_depth = depth[y_mean, x_mean]
#
#         timer.toc()
#        # span_grasp = range(len(poss))
#         grasp_available = False
#         # if depth value is 0, then pass
#         '''
#         for i in range(len(poss)):
#             grasp_index = random.choice(span_grasp)
#             pixel_x, pixel_y = poss[grasp_index]
#             theta = angs[grasp_index]
#             ddd = depths[grasp_index]
#             grasp_depth = depth[pixel_y, pixel_x]
#             if grasp_depth !=0:
#                 grasp_available = True
#                 break
#         '''
#         if not grasp_available:
#             continue
#         # plot the grasp prediction
#         # plotGrasp(rgb, poss[grasp_index], theta)
#         print('...........', x_mean, y_mean, theta, grasp_depth)
#         cv2.imshow('display', rgb)
#         cv2.moveWindow('display',10,10)
#         # print q
#         print('All detection time is %f:' % (timer.diff))
#         q = cv2.waitKey(600)
#         cv2.destroyAllWindows()
#         if q&0xFF == ord('q'):
#             continue
...
#         # pdb.set_trace()
#         # ur5 grasp
#         ur5.soft_grasp([x_mean,y_mean,grasp_depth,theta+8]) # 8 is offset for grasp theta is 0
#         # move ur5 to home position
#         ur5.move_pose(grasp_home)
#         # place the fruit at random position
#         x, y = random.randint(440,580), random.randint(122,374)  # random bin position
#         # x, y, random_theta = random.randint(75, 335), random.randint(138, 368), random.randint(0,180)  # random place position
#         ur5.soft_place([x,y,0.88,0,0])
#         ur5.move_pose(grasp_home)
#         grasp_num +=1
#         print(grasp_num)
#         continue
#         # continue the loop, bottom code is valid
#         pdb.set_trace()
#
#         #pdb.set_trace()
#         plt.close()
...
#         plt.text(bbox[0], bbox[1] - 2, class_name,
...
#             continue
#         ur5.pick([px,py,z])
#         box='left_box' if class_name in left_box else 'right_box'
#         ur5.place(box)
#         num_pick+=1
#         logger.record_result(cv2.cvtColor(rgb,cv2.COLOR_BGR2RGB),num_pick,[class_name,bbox],all_results,[px,py],all_points)
#         logger.save_image(rgb,depth,num_pick)
#         time_one=timer.toc()
#         print('<<<<<<<<<<<<<<<< Pick one object takes %f s' % timer.diff)
#         print('<<<<<<<<<<<<<<<< Pick one object takes average %f s' % time_one)
#         print('<<<<<<<<<<<<<<<< Pick %d objects takes  %f s' % (num_pick, timer.total_time))
#     plt.ioff()
#     plt.close()
#     #plt.show()
#     with open(os.path.join(logger.path,'result.txt'),'w') as f:
#         f.write('<<<<<<<<<<<<<<<< Pick one object takes average %f s \n' % time_one)
#         f.write('<<<<<<<<<<<<<<<< Pick %d objects takes  %f min' % (num_pick, timer.total_time/60.0))
#     print('PICKING DONE')
#     print('<<<<<<<<<<<<<<<< Pick one object takes %f s' % timer.diff)
#     print('<<<<<<<<<<<<<<<< Pick one object takes average %f s' % time_one)
#     print('<<<<<<<<<<<<<<<< Pick %d objects takes  %f min' % (num_pick, timer.total_time / 60.0))
#
#
# if __name__=='__main__':
#     main()
